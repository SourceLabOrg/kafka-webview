...
import com.darksci.kafka.webview.manager.kafka.dto.TopicConfig;
import com.darksci.kafka.webview.manager.kafka.dto.TopicDetails;
import com.darksci.kafka.webview.manager.kafka.dto.BrokerConfig;
...
import com.darksci.kafka.webview.manager.kafka.dto.PartitionDetails;
...
public class KafkaOperations implements AutoCloseable {
    ...
    public TopicList getAvailableTopics() {
        final List<com.darksci.kafka.webview.manager.kafka.dto.TopicListing> topicListings = new ArrayList<>();

        try {
            final Collection<TopicListing> results = adminClient.listTopics().listings().get();
            for (final TopicListing entry: results) {
                topicListings.add(
                    new com.darksci.kafka.webview.manager.kafka.dto.TopicListing(entry.name(), entry.isInternal())
                );
            }
