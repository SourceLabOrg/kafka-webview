...
public class KafkaOperations implements AutoCloseable {
    ...
    public TopicList getAvailableTopics() {
        final List<com.darksci.kafka.webview.ui.manager.kafka.dto.TopicListing> topicListings = new ArrayList<>();

        try {
            final Collection<TopicListing> results = adminClient.listTopics().listings().get();
            for (final TopicListing entry: results) {
                topicListings.add(
                    new com.darksci.kafka.webview.ui.manager.kafka.dto.TopicListing(entry.name(), entry.isInternal())
                );
            }
